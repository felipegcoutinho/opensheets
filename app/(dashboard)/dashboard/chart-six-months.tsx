...
type Props = { data: { month: string; incomes: number; expenses: number; balanco: number }[] };
    ...
    <CardContent className="p-0">
      <ChartContainer className="my-8" config={chartConfig as any}>
        <BarChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
          ...
          <ChartTooltip cursor={{ fill: "rgba(0,0,0,0.05)" }} content={<ChartTooltipContent valueFormatter={formatCurrency} indicator="line" className="w-52 p-2 shadow-md" />} />
          <ChartLegend content={<ChartLegendContent />} />

          <Bar dataKey="incomes" fill={chartConfig.incomes.color} radius={[4, 4, 0, 0]} />
          <Bar dataKey="expenses" fill={chartConfig.expenses.color} radius={[4, 4, 0, 0]} />
          <Bar dataKey="balanco" fill={chartConfig.balanco.color} radius={[4, 4, 0, 0]} />
        </BarChart>
      </ChartContainer>
    </CardContent>
  );
