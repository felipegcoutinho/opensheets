...
export type PainelData = {
  ...
  categoryData: { tipo_transacao: string; categoria: string; total: number; id: string; icone?: string }[];
  conditions: any[];
  payment: any[];
};

export const buildPainelData = cache(async (month: string): Promise<PainelData> => {
  const {
    bills,
    invoiceList,
    recentTransactions,
  ...
  const previous = financialResumeByPreviousMonth?.[0] || { saldo_previsto: 0 };
