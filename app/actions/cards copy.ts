...
export async function getLimite(cartoes_ids) {
  const supabase = createClient();

  // Obter despesas em aberto para múltiplos cartões
  const { error, data } = await supabase
    .from("transacoes")
    .select("cartao_id, valor")
    .in("cartao_id", cartoes_ids)
    .eq("tipo_transacao", "Despesa")
  ...
  const despesasPorCartao = cartoes_ids.reduce((acc, cartao_id) => {
    const despesas = data
      .filter((item) => item.cartao_id === cartao_id)
      .reduce((sum, item) => sum + parseFloat(item.valor), 0);

    acc[cartao_id] = despesas;
    return acc;
  }, {});

  // Obter limites totais dos cartões
  const { error: errorCartoes, data: cartoes } = await supabase
    .from("cartoes")
    .select("id, limite")
    .in("id", cartoes_ids);

  if (errorCartoes) {
    console.error("Erro ao buscar limites dos cartões:", errorCartoes);
    return null;
  }
  ...
  return cartoes.map((cartao) => ({
    ...
    limiteEmAberto: despesasPorCartao[cartao.id] || 0,
    limiteDisponivel: cartao.limite - (despesasPorCartao[cartao.id] || 0),
  }));
}
